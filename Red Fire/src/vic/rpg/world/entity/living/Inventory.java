	...
	{	
		...
		for(CompoundTag t : slotList)
		{
			...
			if(idTag != null)
			{
				...
				Item i = WorldRegistry.itemRegistry.get(id);
		...
		for(CompoundTag t : skillList)
		{
			...
			{
				int id = (Integer)idTag.getValue();
				Skill s = WorldRegistry.skillRegistry.get(id).clone();
				skills.put(slotID, s);		
			}
		...
		for(CompoundTag t : slotGridList)
		{
			...
			for(Tag t2 : (List<Tag>)tMap.get("itemList").getValue())
			{
				...
				Item it = WorldRegistry.itemRegistry.get((Integer)itemMap.get("id").getValue());
